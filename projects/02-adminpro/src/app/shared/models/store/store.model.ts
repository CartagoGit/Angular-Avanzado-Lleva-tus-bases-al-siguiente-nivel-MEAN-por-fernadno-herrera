...
import { StoreOptions, StoreParams } from '../../interfaces/models/store.interface';
...
export class Store<T extends { [key in keyof T]: T[key] }> {
   ...
   public setState = (newState: T): void =>
      this._zone.run(() => this.observer.next(newState));
   ...
   public resetState = (): void => {
      this._zone.run(() => {
         this.observer.next(this.firstState);
      });
   };
   ...
   public reloadState = (): void => {
      ...
      this._zone.run(() => this.observer.next(this.getState()));
   ...
   public getParam = <
      P extends keyof T,
      K extends { needDestructure?: boolean }
   >(
      param: P,
      options?: K
   ): K extends {
      ...
      ? T[P]
      : Readonly<T[P]> => {
      const { needDestructure } = options || {};
      if (needDestructure) {
         if (Array.isArray(this.observer.value[param]))
            return [...this.observer.value[param]] as T[P];
         return { ...this.observer.value[param] };
      }
      return this.observer.value[param];
   };
   ...
   public endState = (): void => {
      if (this.observer.closed && !this.observer.observed) return;
